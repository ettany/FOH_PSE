    ...
    try:
        ...
        if user and check_password_hash(user['password'], password):
            ...
            if username == "administration":
                ...
                response = make_response(jsonify({"message": "User login successful!", "user": username, "totalCash": user["totalCash"]}), 200)
                # Set cookies
                response.set_cookie(key='username', 
                                    value=username, 
                                    httponly=True,  # Optional, prevents JavaScript access if security is required
                                    samesite='None',  # Required for cross-origin
                                    secure=True  # Use HTTPS
)
                response.set_cookie(key='totalCash', 
                                    value=str(user["totalCash"]),  # Convert to string if it's a number 
                                    httponly=True,
                                    samesite='None',
                                    secure=True
        )
